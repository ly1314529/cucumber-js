function SummaryFormatter(options) {
  ...
  self.storeAmbiguousStepResult = function storeAmbiguousStepResult(stepResult) {
    ...
    table.push.apply(table, stepDefinitions.map(function (stepDefinition) {
      ...
      return [pattern, colors.comment('# ' + relativeUri + ':' + line)];
    }));
    failures.push({
  ...
  self.logIssue = function logIssue(number, stepResult, message, colorFn) {
    ...
    var line1 = number + ') ' + scenarioTitle + ' ' + colors.comment('# ' + scenarioLocation);
    ...
    if (stepDefintion) {
      ...
      line2 += ' ' + colors.comment('# ' + stepDefintionLocation);
    }
    var header = line1 + '\n' + indent(line2, 1);
    self.log(header + '\n' + indent(colorFn(message), 2) + '\n\n');
  };
